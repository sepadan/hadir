...
- **Keupayaan log masuk idMe** (`src/moeis/keupayaan.mjs`): `automatik: true`,
  ...
  (manusia log masuk sendiri). Dari UI tetapan tempatan, log masuk manual itu
  berlaku **dalam WebView2 HADIR Desktop yang sama** — butang
  `btnLogMasukManual` menavigasi terus ke `https://idme.moe.gov.my/login`;
  tiada tetingkap Edge berasingan dibuka.
...
- **Tiada kawalan enjin; tetapi log masuk idMe + penghantaran MOEIS kini
  ...
  `_webView`/`_banner`/`_statusStrip`). Sejak 1.0.10, WebView2 memuat
  `https://idme.moe.gov.my/login` terus pada pelancaran biasa, tanpa menunggu
  tugasan backend; membuka halaman itu **tidak** menaip kredensial atau
  menghidupkan auto-login/auto-hantar. Origin idMe HTTPS dibenar untuk
  navigasi manual walaupun `LoginAuto` mati, manakala MOEIS kekal berpagar
...
- **Mod dev portal sebenar (lalai MATI, `HADIR_DEV_REAL_PORTAL=1`).** Halaman
  ...
  `webview2-dev-<port>`, fail port); mod normal memuat idMe terus sejak
  1.0.10 tetapi tetap **tiada** `--remote-debugging-port` (ujian
  `verify-normal-mode.mjs` hanya memeriksa CDP/port, bukan URL halaman mula).
...
- **Opt-in penghantaran MOEIS milik pemilik (lalai MATI).** Togol
  ...
  menyimpan tetapan.)
- **Pengesahan MOEIS (`Sahkan`) dan gerbang `#statusBadge`.** Dialog simpan MOEIS
  menawarkan DUA butang: `.simpan` ("Simpan") dan `.simpansah` ("Simpan &
  Sahkan"). Menekan "Simpan" sahaja meninggalkan rekod dalam keadaan
  **MENUNGGU PENGESAHAN** — data betul, bendera pelayan
  `rekodSahHadirBulanan` tidak pernah ditetapkan. Aliran desktop dipandu
  `TugasanPenghantaran.Sahkan` (produksi: `true`), dan seam
  `IDomMoeis.StatusBadgeDisahkan()` membaca `#statusBadge` == "TELAH DISAHKAN"
  pada DUA titik berlainan, masing-masing dengan tugas sendiri:
  1. **Sebelum** — laluan pantas `tidak-berubah` tidak boleh memintas
  ...
  2. **Selepas** — dalam baca-semula wajib, `VerifikasiPenghantaran.PengesahanPelayan`
     ...
     pengesahan oleh pelayan; jika data padan tetapi badge tidak bertukar,
     hasilnya `tersimpan` (bukti `pengesahan-pelayan-tiada`) — direkod, tidak
     dilepaskan, tidak dihantar semula secara automatik.

  Badge hanya dibaca apabila `Sahkan` HIDUP (litar pintas `&&`, dikunci ujian
  pada kedua-dua titik). `disahkan` kini bermakna: portal hidup mengesahkan
  identiti + kategori + sebab setiap murid **dan** bendera pengesahan pelayan
...
- **Allowlist navigasi dibina semasa MULA, bukan hanya selepas dialog.** Senarai
  origin ialah satu fungsi tulen (`MainForm.OriginsNavigasi`): origin idMe HTTPS
  **sentiasa** dibenar untuk paparan/log masuk manual sejak 1.0.10, origin MOEIS
  ditambah hanya apabila `LoginAuto` HIDUP, dan origin tambahan mod dev
  portal-sebenar hanya apabila modnya aktif. Loopback dibenarkan oleh
  `NavigationGuard` sendiri. Penjaga dibina dalam pembina `MainForm` DAN
  selepas dialog ditutup; auto-login masih memerlukan opt-in + tugasan menunggu.
...
|---|---|---|---|
| 25 September 2026 | 2.29 / web 1.11.39 | **Segar semula status MOEIS selepas masuk semula ke Semak.** Jika guru keluar dan membuka semula pane Semak ketika permintaan bukti MOEIS lama masih berjalan, panggilan baharu kini ditandai hanya apabila snapshot tarikh/pane/data sudah berubah; selepas permintaan lama tamat, status semasa dimuat semula hanya jika pane Semak masih terbuka; jika pengguna keluar sekali lagi, permintaan ditangguh sehingga pane dibuka semula, dan respons lapuk tetap diabaikan. Permintaan pendua dalam konteks sama kekal digabungkan. Pengesahan nilai mentah `0.4` yang dipaparkan sebagai `0` kekal fail-tertutup di backend. | Ujian regresi gagal pada urutan keluar/masuk semula (hanya 1 panggilan, tiada lukisan) sebelum pembetulan dan lulus selepasnya; `node --test tests/*.cjs` lulus 42/42, semakan sintaks dan diff bersih. Ujian backend mengesahkan `0.4` mentah bukan selesai. Tiada data sebenar diubah. Kod: commit `7292e10`; GitHub Pages: workflow `36133557391` berjaya; aset live 1.11.39 disahkan HTTP 200; paparan telefon 390 px menunjukkan "Selesai MOEIS" hijau tanpa token admin atau panggilan API sebenar. |
| 25 September 2026 | 2.28 / PWA 1.11.38 | **Status MOEIS tanpa log masuk admin.** Respons awam `init` dan `semakKehadiran` hanya menambah boolean `moeisSelesai`, benar hanya jika job berstatus `berjaya`, bilangannya sepadan, setiap rekod tidak hadir bernilai mentah tepat `0` serta kategori/sebab semasa sah; nilai paparan kabur (contoh `0.4` dipapar `0`) gagal-tertutup. Jika admin mengubah sebab selepas job berjaya, status kejayaan dibatalkan dan job perlu dihantar semula secara manual. PWA kad Semak Kehadiran menggunakan bukti pelayan ini tanpa panggilan admin; nama/ID/mesej job tidak didedahkan. Cache init dinaikversi dan dibatalkan sebaik keputusan job direkod. | Ujian sintetik mengesahkan kad kelas memaparkan "Selesai MOEIS" tanpa sesi admin; status bukan berjaya, bilangan tidak sepadan, nilai mentah `0.4` yang dipapar `0`, atau kategori/sebab tidak sah kekal bukan selesai. Tiada rekod sebenar ditulis oleh ujian. Kod: commit `b7524b5`; Apps Script: deployment sedia ada `@124`, `HadirWeb.js` sepadan dengan bacaan-semula; POST smoke tarikh tidak sah memulangkan ralat dijangka tanpa akses data; GitHub Pages: run `36129040437` berjaya, aset dan paparan telefon 390 px disahkan live di `https://sepadan.github.io/hadir/`.
| 25 September 2026 | 2.27 / desktop 1.0.14 | **Kemas kini dalam aplikasi.** Menu dulang "Semak kemas kini…" membaca manifest awam `desktop/kemas-kini/latest.json` (GitHub Pages) dan menawarkan kemas kini apabila versinya lebih baharu. `ManifestKemasKini` menolak manifest rosak, versi tidak sah, URL bukan HTTPS / hos bukan keluaran GitHub / ada userinfo / port bukan lalai, cincang bukan 64 heks, dan saiz melebihi 400 MB. Muat turun → sahkan panjang + SHA256 → hanya kemudian dinamakan sedia-pakai; yang gagal dipadam. Pemasangan diserahkan kepada `update.ps1 -Sumber <fail>` yang sudah terbukti (hentikan → sandaran → salin → sahkan → lancar semula). Proses dilancarkan hanya dari `PemasangKemasKini.cs` (powershell.exe, `UseShellExecute=false`, skrip dalam folder pemasangan); penjaga sumber `KodDesktop_TidakMelancarkanPelayarLuar` dikemas kini dengan pengecualian sempit itu. `setup.ps1` menyalin `update.ps1` + `hentikan-hadir.ps1` ke folder pemasangan supaya PC sekolah tidak perlu folder repo. | `dotnet test desktop/HadirDesktop.sln`: 952 lulus (termasuk senarai hos, cincang, saiz, muat turun HTTP palsu yang sahih/tidak sahih, dan hujah pemasangan). Release GitHub `desktop-v1.0.14` telah diterbitkan; exe, ZIP pemasang dan manifest dimuat turun semula, saiz serta SHA-256 sepadan dengan fail tempatan. Kemaskini melalui butang dalam aplikasi belum diuji hujung-ke-hujung. |
| 25 September 2026 | 2.26 / desktop 1.0.13 | Pakej pemasang PC sekolah: `desktop/installer/bina-pemasang.ps1` membina `dist\HadirDesktop-Pemasang-<versi>.zip` + manifest JSON (versi, saiz, SHA256 exe dan zip) daripada exe yang diterbitkan. Pakej mengandungi **hanya** exe, `setup.ps1`, `update.ps1`, `hentikan-hadir.ps1`, `KEMASKINI.md`, `PASANG.cmd` (klik dua kali) dan `BACA-SAYA.md`; skrip gagal-tertutup jika ada fail tambahan atau fail bernama sensitif (`*.dat`, `rahsia*`, `kredensial*`, `*token*`). Tiada rahsia, kredensial DPAPI atau fail Companion dimasukkan — setiap PC menaip URL dan rahsia enjinnya sendiri. `desktop/installer/BACA-SAYA.md` menyenaraikan langkah pemasangan, keperluan WebView2, dan larangan menyalin fail DPAPI antara PC. | Pakej 1.0.13 dibina dan diperiksa isinya (7 fail, tiada fail sensitif); SHA256 exe sepadan dengan exe yang dipasang. Tiada PC sekolah sebenar disentuh. |
| 25 September 2026 | 2.25 / desktop 1.0.13 | Klaim ialah identiti **muktamad** tugasan: aliran membina tugasan penghantaran daripada muatan klaim sahaja (kelas + senarai murid yang dibaca backend di bawah `ScriptLock`), dan **tidak pernah** menggantikannya dengan snapshot senarai yang lebih lama. Klaim yang membawa senarai murid kosong atau kelas kosong kini gagal-tertutup: pembina menolaknya, klaim dilepaskan, tiada murid ditanda, tiada tulisan portal dan tiada laporan keputusan. Sebelum ini baris `klaim.Murid.Count > 0 ? klaim.Murid : kerja.Murid` membenarkan snapshot lama ditulis semula — dibuktikan MERAH oleh ujian regresi (`tanda:102` berlaku walaupun klaim kosong) sebelum dibaiki. | `dotnet test desktop/HadirDesktop.sln`: 898 lulus, termasuji klaim kosong, klaim tanpa kelas, dan kawalan klaim penuh. Folder sementara, DPAPI rekaan dan backend palsu; tiada portal, rangkaian, kredensial sebenar atau rekod kehadiran disentuh. |
...
| 23 September 2026 | 1.11.35 (desktop 1.0.7 · Apps Script @119) | **Punca akhir `tidak-berubah` palsu + aliran automatik penuh (simpan → MOEIS) tanpa menekan "Hantar".** Pengguna melaporkan 1 BIJAK `berjaya` tetapi MOEIS kosong, dua kali (16:01, 16:10). Siasatan menemui dua kecacatan: (1) `NavigasiHarian()` melepaskan `wv.Navigate(...)` dan menunggu **jeda tetap** `Delay(_masaMuatMs)` (4 s) — bukan `NavigationCompleted` — jadi bacaan BERLUMBA dengan navigasi dan boleh mendarat pada DOM LAMA (kotak tak-tanda daripada percubaan terdahulu → `perluTanda` kosong → `tidak-berubah → berjaya`); kegagalan tab 15:40 (`halaman-tidak-sedia`) ialah gejala sama: MOEIS belum render dalam 4 s. Ini kelas bug **sama** seperti `MuatSemula()` pagi tadi. Pembetulan: `NavigasiHarian` kini langganan → `MulaNavigasiAsync(url)` → tunggu `NavigationCompleted` sebenar melalui `PengendaliMuat` (fail `PelayarMuat.cs` kini menerima callback `mula` pilihan; default kekal `MulaMuatSemulaAsync`), kemudian `Delay(_jedaAjaxMs)` hanya bila `Selesai`; gagal/tamat masa **dipulangkan** (bukan dilempar — kontrak seam: "setiap kaedah tidak melontar") dan aliran menilainya SEBELUM membaca DOM, memberi `halaman-tidak-sedia` + `navigasi-gagal`/`navigasi-tamat-masa` yang jujur. (2) Tiada laluan automatik: kini `hadirSimpanKehadiran_` memanggil `hadirMoeisJobBuatDalaman_` **selepas** `lock.releaseLock()` (Apps Script lock tidak reentrable — teras mengambil lock sendiri) dan teras dipecahkan daripada `hadirMoeisJobBuat_` supaya **bebas sesi** (guru menyimpan kehadiran tidak memegang sesi admin); kegagalan mencipta tugasan hanya dilog (`MOEIS_JOB_AUTO_GAGAL`) dan TIDAK boleh menggagalkan simpanan guru. Simpanan baharu memulihkan tugasan terkunci oleh positif palsu kerana gate v118 membenarkan `berjaya` dicipta semula. | Ujian desktop **662 / 662 lulus** (658 asal + 4 ujian navigasi baharu: langganan mendahului navigasi + URL kekal, gagal ≠ senyap, tamat masa tidak gantung, default callback kekal semula-muat); dua ujian sapuan refleksi marshaller mengekalkan kontrak "tidak melontar" (pelaksanaan berpindah daripada `throw` kepada nilai pulangan). Desktop 1.0.7 dipasang — SHA256 `F81A6A60BFB4E8563328E93ED89ADE2BFB81E2A9A44E46611F7CC0A118B7632B`; rollback `.bak-1.0.6`. Apps Script: `clasp push` (fail kerja `hadir-clasp-prod`, BUKAN `hadir-gas` yang basi 18 Sep, BUKAN fail repo yang mengandungi ciri `pc*` belum dideploy) + `clasp deploy -i <id sama>` → **`@119`**, URL `/exec` tidak berubah. |
| 23 September 2026 | 1.11.35 (web) | **Butang Hantar dilumpuhkan pada status `berjaya` di pelayar.** Selepas gate backend dibuka (v118), pengguna menekan Hantar pada 1 BIJAK dan tiada apa-apa berlaku — kerana `app.js:1357` menyekat `statusPenghantaran === 'berjaya'` (gerbang lama yang sama, cuma di sisi pelayar) dan backend tidak pernah menerima permintaan itu. Kini ungkapan `hantarBtn.disabled` hanya menyekat `!lengkap`, `!bilTidakHadir` dan tiga status dalam penerbangan (`menunggu`/`sedang_dihantar`/`tersimpan`) — sepadan tepat dengan gate Apps Script v118. Penjaga "belum lengkap" dikekalkan. Cache dipaksa buang: versi `1.11.34` → `1.11.35` di `config.js`, `index.html` (4 rujukan `?v=`), `service-worker.js` (`CACHE_VERSION …-20260923-2` + 4 URL aset) supaya `app.js` lama tidak disajikan semula. | Ujian: `hadir.test.cjs` EXIT=0 + penjaga baharu (ungkapan `disabled` tidak boleh mengandungi `'berjaya'`, mesti kekal mengandungi tiga status penerbangan); `hadir-pc-vm` 30/30. Pembaikan ini melengkapkan rantaian: v118 menerima cipta semula → butang pelayar boleh menghantar → enjin mengambil dalam ~10 minit → pengesahan baca-semula 1.0.4/1.0.6. |
| 23 September 2026 | 1.11.34 (Apps Script v118) | **Gerbang `hadirMoeisBolehCiptaJob_` — status siap kini boleh dihantar semula.** Selepas fix 1.0.4/1.0.6, 1 BIJAK kekal terkunci: barisnya (salah) `berjaya` sejak positif palsu pagi tadi, dan gate asal (`undefined/null/''/'gagal'` sahaja) menolak cipta semula — walaupun MOEIS langsung tidak terisi, tiada jalan keluar automatik. Perhatikan bahawa pemanggil `moeisJobBuat` **menggunakan semula baris yang sama** (id lama, `setValues` semula kepada `menunggu`), jadi pendua memang mustahil — gate itu bukan melindungi pendua, ia hanya menghalang hantar semula. Kini: `['gagal','berjaya']` dibenarkan; `menunggu`/`sedang_dihantar`/`tersimpan` KEKAL disekat supaya lease enjin tidak direset di tengah jalan. Jalan pemulihan: admin menekan **Hantar** semula → backend menerima → enjin mengambilnya dalam ~10 minit → hantar + pengesahan baca-semula (1.0.4/1.0.6) → `berjaya` hanya jika MOEIS benar-benar padan. | Deploy: `clasp deploy -i AKfycbzqppw…hbe` (deployment **sama**, `@117` → `@118`; URL `/exec` tidak berubah). Baca semula kod hidup mengesahkan gate baharu; asap ujian sebenar `{"mode":"hadir","kaedah":"init"}` → `HTTP 200, 27,186 bait, ok:true`. Ujian: web `hadir.test.cjs` EXIT=0 (baris 293 diterbalikkan + dua status penerbangan ditambah), VM `hadir-pc-vm.test.cjs` 30/30 (ujian gerbang baharu). NOTA deploy: `appscript-push/` ialah bundle **Jadual**, bukan HADIR — jangan campur. |
| 23 September 2026 | 1.11.34 (desktop 1.0.5-1.0.6) | **Shell desktop: bukti produksi + `tidak-berubah` mengulang tanpa henti.** Selepas pembetulan positif palsu, ujian hidup 2 BIJAK (14:07 dan 14:15) melaporkan `1 dicuba, 0 dilangkau; sekurang-kurangnya satu tidak disahkan — semak bukti` tetapi TIDAK menulis sebarang langkah — `MainForm.cs:155` menyambungkan callback langkah hanya bila `HADIR_DEV_AUTO_KITARAN` hidup, jadi produksi menerima `null`, dan `DevAutoKitaran.LaluanLog` juga `null` apabila mod dev mati (gerbang dua lapis). "semak bukti" pada PC sekolah menunjuk ke log yang tiada. (1.0.5) Langkah kini ditulis TANPA SYARAT ke log produksi `hadir-desktop.log` — kandungannya hanya id tugasan / kelas / status / sebab, tiada kredensial, IC, token atau URL bertoken. Langkah itu segera mendedahkan keadaan sebenar: `14:21:50 status=tidak-berubah sebab=MOEIS sudah menanda setiap murid dalam tugasan sebagai tidak hadir` — penghantaran MEMANG berjaya, ia hanya gagal diberi label. (1.0.6) Puncanya: apabila `perluTanda.Count == 0` (semua murid dijangkakan sudah tidak hadir di MOEIS, dikira daripada jadual portal HIDUP selepas pagar konflik), `HasilPenghantaran.Berjaya` kekal `false`, lalu pelaporan jatuh ke `LepasSenyapAsync` — tugasan dibebaskan TANPA rekod dan diulang setiap 10 minit selamanya. Kini `Berjaya = true`: keadaan yang diingini memang telah disahkan oleh bacaan langsung, jadi laporan ialah `keputusan=berjaya` dengan sebab jujur "tiada perubahan dihantar". | Tiada data murid. 658 lulus / 0 gagal; ujian `SemuaSudahTidakHadir_TiadaSimpanLangsung` dikemas kini — penjaga sebenarnya (`BilKemaskini == 0`, `BilSimpan == 0`, tiada simpan) KEKAL tak disentuh, hanya assert `Berjaya` diterbalikkan. Bukti hidup 2 BIJAK: `14:32:06 SELESAI_OK keputusan=berjaya` + `semua disahkan`. |
